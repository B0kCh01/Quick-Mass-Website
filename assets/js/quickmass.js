...
function isCapital(inputChar) {
    return inputChar == inputChar.toUpperCase() ? true : false;
}
...
function getMass(rawInput) {
    if (isNaN(rawInput[0]) == false) {
    ...
    try {
    	return "Molar Mass: " + eval(tokenList.join(""));
    } catch (SyntaxError) {
    	return "Invalid Compound - Check for spelling"
    }
}
...
$("#name").on("keyup paste", function() {
    ...
    if ($("#name").val() == "") {
        ...
        document.getElementById("molar-mass").innerHTML = (getMass($("#name").val()));
    }
});